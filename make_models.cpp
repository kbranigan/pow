...
/////////////////////////////
std::string database = "gcsd000a10a_e";  // better be able to log in as root with no password
/////////////////////////////
...
void populate_only_tables()
{
  ...
  if (database == "gcsd000a10a_e")
  {
    only_tables.push_back("DBF");
  }
}
...
std::string getClassNameFromTableName(std::string val) // translates users => User, stops => Stop
{
  ...
  for (std::map<std::string, std::string>::iterator it = pluralize_exceptions.begin() ; it != pluralize_exceptions.end() ; it++)
  {
    if (val == it->second)
    {
      cc = it->first;
...
/*std::string singularize(std::string val)
{
  for (std::map<std::string, std::string>::iterator it = pluralize_exceptions.begin() ; it != pluralize_exceptions.end() ; it++)
  {
    if (val == it->second)
...
int main(int argc, char **argv, char **envp)
{
  ...
  fp = fopen(query, "w");
  fprintf(fp, "\n/* This file was generated by pow, so it's probably best not to expect edits to stick. */\n\n");
  for (std::vector<std::string>::iterator it = table_names.begin() ; it != table_names.end() ; it++)
  {
    std::string table_name = (*it).c_str();
    if (invalid_table(table_name)) continue;
  ...
  for (std::vector<std::string>::iterator it = table_names.begin() ; it != table_names.end() ; it++)
  {
    ...
    for (unsigned int i = 0 ; i < tables[table_name].assocs.size() ; i++)
    {
      ...
      if (a->type == HAS_ONE)
        fprintf(fp, "  struct %s *%s;\n", getClassNameFromTableName(a->table_name).c_str(), a->table_name.c_str());
      else if (a->type == HAS_MANY)
        fprintf(fp, "  struct %s **%s;\n"
                    "  int num_%s;\n", getClassNameFromTableName(a->table_name).c_str(), a->table_name.c_str(), a->table_name.c_str());
    ...
    /*for (unsigned int i = 0 ; i < tables[table_name].assocs.size() ; i++)
    {
      sAssoc *a = &tables[table_name].assocs[i];
      if (a->type == HAS_ONE)
        fprintf(fp, "struct %s;\nstruct %s* get%s(struct %s *self) {\n  return NULL;\n};\n", 
          getClassNameFromTableName(a->table_name).c_str(), getClassNameFromTableName(a->table_name).c_str(), getClassNameFromTableName(a->table_name).c_str(), getClassNameFromTableName(table_name).c_str());
      else if (a->type == HAS_MANY)
        fprintf(fp, "struct %s;\nstruct %s* get%s(struct %s *self) {\n  return NULL;\n};\n", 
          getClassNameFromTableName(a->table_name).c_str(), getClassNameFromTableName(a->table_name).c_str(), pluralize(getClassNameFromTableName(a->table_name)).c_str(), getClassNameFromTableName(table_name).c_str());
    }*/
    ...
    fprintf(fp, "  if (mysql_query(&mysql, \"SELECT `id`%s ", tables[table_name].fields.size() > 1 ? "," :"");
    for (unsigned int i = 0 ; i < tables[table_name].fields.size() ; i++)
    {
      sField *f = &tables[table_name].fields[i];
    ...
    fprintf(fp, "    \"}\",\n");
    fprintf(fp, "     %s[id].id%s\n", table_name.c_str(), tables[table_name].fields.size() > 1 ? "," : "");
    for (unsigned int i = 0 ; i < tables[table_name].fields.size() ; i++)
    {
      sField *f = &tables[table_name].fields[i];
      if (f->type == "char")
  ...
  fprintf(fp, "  \n"              
              ...
              "  printf(\"web server available at http://localhost:4444/\\n\");\n"
              "  printf(\"-------------------------------------------------------------------------\\n\");\n"
              "  for (;;) sleep(10000);\n"
              "  mg_stop(ctx);\n"
              "}");
